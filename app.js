const STORAGE_KEYS = {
  THEME: "shopLangVietThemeV2",
  USERS: "shopLangVietUsersV2",
  CURRENT_USER: "shopLangVietCurrentUserV2",
  CART: "shopLangVietCartV2",
  ORDERS: "shopLangVietOrdersV2",
  TOPUPS: "shopLangVietTopupsV2"
};
...
function bindAddToCartButtons(scope = document) {
  scope.querySelectorAll("[data-add-service]").forEach((button) => {
    button.addEventListener("click", () => addToCart(button.dataset.addService));
  });
}
  ...
  if (!items.length) {
    view.innerHTML = `
      <section class="card empty-state">
        ...
        <div class="row" style="justify-content:center;margin-top:14px;">
          <a class="btn btn-primary" href="games.html">Xem game</a>
          <a class="btn btn-soft" href="services.html">Xem tất cả dịch vụ</a>
        </div>
      </section>
    `;
    return;
    ...
    <div class="grid-2">
      ...
      <section class="form-card">
        ...
        <form id="checkoutForm">
          ...
          <div class="notice">
            Nếu chọn <strong>số dư ví</strong>, hệ thống sẽ trừ tiền ngay trong demo.
            Nếu chọn <strong>thủ công</strong>, đơn sẽ tạo ở trạng thái chờ thanh toán.
          </div>
  ...
  if (!topups.length) {
    target.innerHTML = `
      <section class="card empty-state">
        ...
        <p class="muted">Hãy thử nạp demo bằng MoMo hoặc thẻ cào bên dưới.</p>
      </section>
    `;
    return;
  }
    ...
    <div class="grid-2">
      ...
      <section class="form-card">
        ...
        <form id="profileForm">
          <label>
            Tên hiển thị
            <input class="input" type="text" id="profileDisplayName" value="${escapeHtml(
